...
class StableDiffusionPipeline {
    constructor(tvm, tokenizer, schedulerConsts, cacheMetadata) {
        ...
        this.tvm.bindCanvas(document.getElementById('image-display').querySelector("canvas"));
        // VM functions
    ...
    async generate(
        ...
        this.tvm.withNewScope(() => {
            const image = this.vaeToImage(latents, this.vaeParams);
            RGBImage = this.imageToRGBA(image);
            this.tvm.showImage(RGBImage);
        });
        latents.dispose();
        await this.device.sync();
        if (progressCallback !== undefined) {
...
class StableDiffusionInstance {
     ...
     * Run generate
     */
    async generate(prompt, negPrompt, schedulerId, vaeCycle) {
        ...
        try {
            ...
            console.log("VAE Cycle: " + vaeCycle);
            await this.pipeline.generate(prompt, negPrompt, this.#getProgressCallback(), schedulerId, vaeCycle);
        } catch (err) {
